...
export function MemoryCollection({ memories, onOpenMemory }: MemoryCollectionProps) {
  return (
      ...
      {memories.map((memory) => (
        <article className={`scene-manager-memory-card tone-${memory.tone}${memory.canEnterSpace ? " is-ready" : ""}`} key={memory.uiKey}>
          <button
            ...
            disabled={!memory.canEnterSpace || !memory.memoryId}
          />
          ...
          <div className="scene-manager-memory-card-body">
            ...
            <span className={`scene-manager-open-memory-link${memory.canEnterSpace ? "" : " disabled"}`}>
              {memory.canEnterSpace ? "进入空间" : "正在处理"}
...
function MemoryCoverImage({ primaryUrl, fallbackUrl }: { primaryUrl: string | null; fallbackUrl: string | null }) {
