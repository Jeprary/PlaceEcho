...
export class SpatialRuntime {
  ...
  private memoryPresentationHeld = false;

  holdMemoryPresentation(held: boolean): void {
    this.memoryPresentationHeld = held;
    if (!held && this.reachedPresentationActive) this.finishMemoryPresentation();
  }

  private finishMemoryPresentation(): void {
    if (this.disposed) return;
    if (this.reachedResumeTimer !== null) window.clearTimeout(this.reachedResumeTimer);
  ...
  enablePointerMode(): void {
    this.gyroscopeEnabled = true;
    if (this.worldReady && this.splatFormationComplete) this.startGlide();
  }
  ...
  private updateAnchor(elapsedSeconds: number): void {
    ...
    if (proximity === "reached" && this.anchorEncounterArmed) {
      ...
      this.reachedResumeTimer = window.setTimeout(() => {
        if (!this.memoryPresentationHeld) this.finishMemoryPresentation();
